...
class UnetTTSDataset(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.exists(os.path.join(cache_dir, "unet_train_cache.pt")) or rebuild_cache:
            ...
            if len(self.datapoints) > 0:
                torch.save(self.datapoints, os.path.join(cache_dir, "unet_train_cache.pt"))
            else:
                import sys
            ...
            self.datapoints = torch.load(os.path.join(cache_dir, "unet_train_cache.pt"), map_location='cpu')

        self.cache_dir = cache_dir
        self.language_id = get_language_id(lang)
        print(f"Prepared a UnetTTS dataset with {len(self.datapoints)} datapoints in {cache_dir}.")
    ...
    def remove_samples(self, list_of_samples_to_remove):
        ...
        torch.save(self.datapoints, os.path.join(self.cache_dir, "unet_train_cache.pt"))
