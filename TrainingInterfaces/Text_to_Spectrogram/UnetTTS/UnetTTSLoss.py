...
class UnetTTSLoss(torch.nn.Module):
    ...
    def forward(self, mel_before, content_latents, content_latents_pred, gold_spectrograms, spectrogram_lengths, text_lengths, gold_durations, predicted_durations):
        """
        Args:
        ...
        # calculate loss
        l1_loss = self.l1_criterion(gold_spectrograms, mel_before)
        l2_loss = self.mse_criterion(content_latents, content_latents_pred)
        duration_loss = self.duration_criterion(gold_durations, predicted_durations)
